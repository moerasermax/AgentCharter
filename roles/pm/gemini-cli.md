...
### Gemini CLI 預設 `generalist` 自動分包處置 — PM init 必提醒（v0.10.5 加 / signal #55）

> **動機**：dogfood signal #55（2026-05-07 LIVE 實證、CryptoBot 反向接入後 user 親自發現）— Gemini CLI 預設啟用 `generalist` agent、會**自動分包任務**繞過 PM 角色卡控（`multi-role-tracking` + `role-separation`）。本 §3.5（v0.6.0）禁令以為治本「LLM 主動繞路」、實際 Gemini CLI **vendor 預設行為層**才是真主因 — PM 不需「自己想繞」就被自動分包。user LIVE 實證：執行 `/agents disable generalist` 後框架卡控紀律恢復正常。對應 v0.8.2 §設計哲學第 5 條「弱保證項升結構強制」家族新類型（vendor 預設行為層紀律、既有 3 軸：條款層 / 多 actor 互檢 / commit-hook binary 之外的第 4 軸）。

**紀律：PM init 開始時必對採用方提醒**

PM self-instantiation 流程開始時（`core/init-template §3.3.2` step 1 自我介紹之前）、必對採用方主動發出以下提醒：

> 「Gemini CLI 預設啟用 `generalist` agent、會自動分包任務並可能繞過 PM 卡控紀律（dogfood signal #55 LIVE 實證、charter v0.10.5 紀律）。建議現在執行以下 slash command（在 Gemini CLI session 中輸入）：
>
> ```
> /agents disable generalist
> ```
>
> 對齊 charter `role-separation §3.5` 卡控紀律。已執行請告知。」

**採用方回應後處置**：

| 採用方回應 | PM 處置 |
|---|---|
| ✅ 已 disable / 願意 disable | 採用方執行後告知 → PM 繼續 init flow |
| ❌ 拒絕 disable（罕見場景）| 紀錄為已知風險（`<common-memory-root>/state/dogfood_signals.md`）+ PM 後續每次任務派發前必自查「是否被 `generalist` 自動分包拐騙繞卡控」（self_audit 額外項）+ 對齊 `core/audit-rights` 默認待抽驗紀律 |

**`pm-init.toml` 自具象化要求**：PM 自具象化 `.gemini/commands/pm-init.toml` 時、prompt 必含上述提醒（對齊 §3.6 self-instantiation checklist + `core/init-template §3.3.2` step 1 之前的 inline reminder）。

**與 §3.2 派發任務既有實作的設計衝突（v0.10.5 留 follow-up）**：

§3.2「派發任務」Gemini CLI 實作（line 52）寫死 `invoke_agent(agent_name="generalist", prompt="...")` — 與本段 disable 紀律**直接衝突**。**v0.10.5 不修**（user 2026-05-07 明示「提醒就好」、scope 限定）。

候選議程（留 dogfood signal 累積觀察、見 NEXT.md ⚪ 待對話段）：
- 累積 ≥ 2 次「PM disable generalist 後派發機制失效」LIVE 觀察 → 評估 §3.2 重構（改用 explicit cross-AI handoff 取代 sub-agent 派發、對齊 `core/cross-ai-handoff §3` 路徑）
- 並列議程：`core/role-separation §3.5` 繞路禁令在 vendor 預設行為層的覆蓋紀律是否升 core 條款層（dogfood signal #5 + #41 + #55 同 family、新架構級概念候選第 14 個、留 v0.11.0 MINOR 評估）
