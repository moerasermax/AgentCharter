...
| `validator` 角色 | **對稱性（v0.7.0 加）**：auditor 是 charter maintainer 場景的抽驗角色 / validator 是採用方場景的抽驗角色。兩者位階不同（maintainer-only ⊥ 採用方）但**機制同源** — 透過 fresh-context sub-agent / 不同 session / 邀請其他 vendor 達成「他抽」屬性。validator §3.6 init 階段抽驗（v0.7.0 加）對應 auditor 對 maintainer commit 的 cross-reference sweep（封閉「**自抽自驗**」結構性盲區的兩個半邊：maintainer 半邊 = auditor / 採用方半邊 = validator init 階段抽驗）|

---

## 9. 變更歷史

### v0.2（自 v0.7.0 起，patch only）

**動作**：§8 對應其他角色表 `validator` 行加反向引用 — 說明 auditor / validator 在「自抽自驗」封閉的對稱性（maintainer 半邊 / 採用方半邊）。

**觸發**：v0.7.0 dogfood signal #7 候選條款化（採用方接入流程缺 init-validator 角色），對應 `roles/validator/_spec.md §3.6` 加段。本 spec 只加反向引用、職能本體不變。

**修訂類型**：PATCH — 純引用同步、不動 auditor 自身職能。
