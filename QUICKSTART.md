...
- **v1.7（2026-05-23，charter v0.13.0）** — Engineer × Codex Desktop vendor 接入完成 MINOR 連動 sync（邀請制四步驟走完、零採用方動作要求）。**升 v0.13.0 採用方 quick action**：只改 profile.yaml `charter_version: "0.12.0"` → `"0.13.0"`、無其他動作。**升版內容**：新 vendor coverage `roles/engineer/codex.md` v0.1 + `templates/vendor-adapters/codex.skill.tpl` v1.0 — Engineer 角色從 1 vendor 擴為 2 vendor（Claude Code + Codex Desktop）。**Codex Desktop 採用方接入起點**：依 §4 self-instantiation prompt 範本 + 換讀 `roles/engineer/codex.md` 即可（vendor schema 為 Codex Skill `~/.codex/skills/engineer-init/SKILL.md` 子目錄結構）。詳見 CHANGELOG v0.13.0 段。

- **v1.6（2026-05-22，charter v0.12.0）** — 🔥 **3 in 1 BREAKING-MEDIUM ship**：(1) Antigravity vendor 接入 + (2) Canonical Init Spec Layer + (3) agents-commons rename。**升 v0.12.0 採用方 quick action**：(a) 跑 `bash ~/.agentcharter/tools/vendor/commons/migrate-to-agents-commons.sh`（三 phase 互動式 rename `agent-commons/` → `agents-commons/`）；(b) 若用 Gemini CLI 免費/Pro/Ultra — 6/18 前裝 `agy` + `agy plugin import gemini` + 重 self-instantiate；(c) 重 `install-git-hooks --update` + 升 `charter_version: "0.12.0"`。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.6-to-v0.12.0-antigravity-canonical-rename.md`](./examples/upgrades/v0.10.6-to-v0.12.0-antigravity-canonical-rename.md)**。

- **v1.5（2026-05-22，charter v0.11.0、含 v0.10.4 / v0.10.5 / v0.10.6 補記）** — 🔥 **Gemini CLI 棄用觸發 Antigravity CLI vendor 接入 MINOR 連動 sync**（dogfood signal #60 LIVE 條款化候選）。**升 v0.11.0 採用方 quick action**：若你用 Gemini CLI 免費 / Pro / Ultra → 6/18 前裝 `agy` + 跑 `agy plugin import gemini` + **手動轉換** `.gemini/commands/*.toml` → `~/.gemini/skills/<name>/SKILL.md`（子目錄結構、LIVE 第二輪校正）；若 Enterprise / 付費 API key / OSS → 不必動。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md`](./examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md)**。**補記中間 PATCH**（v0.10.4 vendor 介紹工具紀律 / v0.10.5 Gemini generalist disable / v0.10.6 Claude PM v1.0 接入 + Gemini PM v1.8 收斂）對 QUICKSTART 流程零影響、純 vendor spec 擴增。詳見 ADOPTION.md / TUTORIAL.md 變更歷史 + CHANGELOG v0.10.4〜v0.11.0 段。
