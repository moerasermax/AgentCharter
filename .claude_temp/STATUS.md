...
> **更新時間**：2026-04-30（台灣時間，post-v0.9.0 + v0.9.1 PATCH release 收尾 snapshot — dogfood signal #36 Gap 偵測條款化）
> **當前版本**：v0.9.1（**doctor Gap 偵測 + 互動式引導 + Doctor 角色概念層** — 平行獨語反模式偵測 + init scaffold 預防 + post-upgrade verify 後 Doctor 角色建立提示、**嚴守向下兼容紀律**（v0.x 階段、純擴增 tools/spec/roles 層、採用方只需改 `charter_version` 一行、零執行邏輯影響）、**dogfood-driven hardening 第十八循環**：dbSDK LIVE 公司專案 Gap 診斷 → 條款化 + 互動式引導設計）
> **Working tree 狀態**：v0.9.1 修訂完成、待 commit + push
