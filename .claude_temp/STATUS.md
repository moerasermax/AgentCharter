...
> **更新時間**：2026-05-23（台灣時間、v0.13.0 ship 後最小 sync sweep — header + Version 軌跡 + 已對外實證、深層條款數 / 架構級概念數 / preset 表全文 sweep 留下次 session）
> **當前版本**：**v0.13.0**（**MINOR — Engineer × Codex Desktop vendor 接入完成**、邀請制四步驟走完、Engineer 角色 vendor coverage 從 1 → 2、零採用方動作要求）
> **Working tree 狀態**：✅ 已 commit + push（v0.13.0 ship commit `cae8d3e` + docs sync 後續）
> **GitHub**：https://github.com/moerasermax/AgentCharter（private）
> **最後 checkpoint**：本檔為 v0.13.0 ship 後最小 sync（snapshot 從 v0.10.5 跳至 v0.13.0、補 4 個 release 軌跡進 Version 軌跡表頭部；下層細節（21 條條款清單仍寫 21 條 / 13 個架構級概念仍寫 13 個 / 三 preset 仍寫 3 個 / 演化軸表斷在 v0.9.0）為**已知 sync 債**、留下次 session 專門 sweep）
> **Git tags**：`v0.5.9` @ `a24c15c` / `pre-v0.6.0-batch` @ `2225659` / `v0.5.10` @ `6dd3eda` / `v0.6.0` @ `9493814` / `v0.6.1` @ `72caaee` / `v0.7.0` @ `bcbf964` / `v0.7.1` @ `c26b5b4` / `v0.7.2` @ `054e6c7` / `v0.7.3` @ `0468570` / `v0.7.4` @ `130638b` / `v0.7.5` (待打) / `v0.8.0` @ `b5866b5` / `v0.8.1` @ `0cf5494` (待打) / `v0.8.2` @ `a2adecf` (待打) / `v0.9.0` ~ `v0.13.0` (全待打 — tag 補打議程留下次 session)

> ⚠️ **STATUS.md 已知 sync 債清單**（提醒下次接班 AI）：
> 1. **27 條 core 條款清單**（本檔 §「21 條 core 條款清單」段）— 仍寫 21 條 / 缺 v0.9.0 加 4 條（individual-learning-loop / diagnose-remediate-protocol / adoption-lifecycle / condition-mutability）+ v0.12.0 加 2 條（vendor-lifecycle / init-spec-schema）
> 2. **15 個架構級概念**（本檔 §B 段）— 仍寫 13 個 / 缺 v0.12.0 加 2 個（vendor-lifecycle / init-spec-schema）
> 3. **4 個 preset**（本檔 §「三個 preset」段）— 仍寫 3 個 / 缺 v0.9.0 加 essential preset
> 4. **演化軸表**（本檔 §A 段）— 斷在 v0.9.0 / 缺 v0.9.1〜v0.10.6 / v0.11.0 / v0.12.0 / v0.13.0 共 ≥ 15 個 release 軌跡
> 5. **§D 跨議題已釐清盲點段**— 缺 v0.10.x〜v0.13.0 累積的 dogfood signal #36〜#62 完整觀察
> → 建議下次 session 用 `/maintainer-load` 進入後第一個議題就是 STATUS.md 深層 sweep（≥ 100 行改動、≥ 1 session）

---

## Version 軌跡（最新 commit）

| 版本 | Commit | 主題 |
|---|---|---|
| **v0.13.0 docs sync** | （本 commit）| **docs(v0.13.0-sync)** — 內部追蹤層補完：`.claude/commands/maintainer-load.md` 當前狀態段 v0.9.0 → v0.13.0（補 v0.9.1〜v0.13.0 共 ≥15 release 軌跡）+ `.claude_temp/NEXT.md` 加 v0.13.0 已 ship 段 + #62 dogfood signal 候選正式登記 + Codex adapter cross-platform sweep follow-up + STATUS.md 本檔最小 sync（header + Version 軌跡 + 已知 sync 債清單）。對齊 `core/maintainer-discipline §3.4.3` 內部追蹤層 sync checklist |
| **v0.13.0** | `cae8d3e` | **MINOR — Engineer × Codex Desktop vendor 接入完成（邀請制四步驟走完）**：依 `core/ai-vendor-onboarding §3` 完整四步驟（charter 寫概念層 → 邀請 Codex 寫 vendor 層 → Claude Code 既有 vendor 校正 regression 無 regression → maintainer 三層簽收 + 文檔層 sync）。**新檔**：(1) `roles/engineer/codex.md` v0.1（Codex Desktop Engineer vendor 層 spec、7 段結構齊備、附 `codex features list` + 本機 `~/.codex` 結構實測為據、anti-`dogfood signal #41` 教科書級實證 — 拒絕宣稱 `.codex/commands/` 不存在的 native slash command、改用 Codex Skill schema、附本機反證；§5 vendor 預設行為層 7 row 自防對齊 `core/vendor-lifecycle.md`、§6 sub-agent 跨界禁令 5 row 對齊 `role-separation §3.5`、§7 跨 AI 對應段 11 row 表）+ (2) `templates/vendor-adapters/codex.skill.tpl` v1.0（Codex Skill `~/.codex/skills/<role>-init/SKILL.md` target、對齊 README §3 五條 adapter 紀律全綠）。**Engineer 角色 vendor coverage 從 1 → 2**（Claude Code + Codex Desktop）。**設計學意義**：(a) v0.12.0 Canonical Init Spec + Vendor Adapter pattern 首次新 vendor 接入 LIVE 驗證、Codex 一輪即過 step 4 簽收 / (b) v0.12.0 Follow-up 第一項「Cursor / Kiro / Codex 等 vendor 接入」第一個 LIVE ship / (c) **path A / path B 之外的「path C 邀請制 step 2 default 路徑」第一個明確 ship**（charter 寫概念層在前、vendor 應邀寫 vendor 層、無 from-source vendor template、charter 第一個非 from-source 邀請制完整實證）。**dogfood signal #62 LIVE 條款化候選**（global skill version drift、#58 vendor skill abstraction family 延伸）|
| **v0.12.0** | `c94a8b0` | **MINOR — 3 in 1 BREAKING-MEDIUM**：(1) `core/vendor-lifecycle.md` 第 14 個架構級概念（收編 #5/#41/#55/#58/#59/#60 family）+ (2) `core/init-spec-schema.md` 第 15 個架構級概念（SSS S2.5 Canonical Init Spec + Vendor Adapter pattern、解 dbSDK PM AI 報告三 vendor pm-init 差異 604/11842/1645 bytes、signal #61 條款化）+ 3 個 `templates/vendor-adapters/*.tpl`（gemini-cli / claude-code / antigravity）+ `roles/{pm,engineer}/init-spec.md` canonical + (3) agents-commons rename BREAKING-MEDIUM（`agent-commons/` → `agents-commons/` + `migrate-to-agents-commons.sh` 一鍵 migration）。**架構級概念 13 → 15、條款數 25 → 27**。後續兩個 docs sync commit：`834b8bd` + `4c1dfc9` |
| **v0.11.0** | （commit 待查）| **MINOR — Gemini CLI 棄用觸發 Antigravity vendor 接入**（Google 2026-05-19 宣布 Gemini CLI 2026-06-18 對 Pro/Ultra/free 斷線）：`roles/pm/antigravity-cli.md` v1.1（path A AI-DRAFTED-FROM-GEMINI-V1.8 + 同日 LIVE 第二輪校正、Skills 子目錄 `~/.gemini/skills/<name>/SKILL.md`）+ `roles/pm/gemini-cli.md` v1.9 標 LEGACY_AUTO_IMPORTED + walkthrough + UPGRADE.md 顯著警告段。**A1 公理時運實證**（PM 角色不死、Claude PM + Antigravity 雙保險）+ **vendor 中立架構反向實證**（v0.10.0 commit-hook + v0.9.4 handler `tools/vendor/commons/` survive vendor death）。**dogfood signal #60 LIVE 條款化候選** |
| **v0.10.6** | （commit 待查）| **PATCH — Claude PM v1.0 接入 + Gemini PM v1.8 best-of-breed 收斂升級**：(a) 新檔 `roles/pm/claude-code.md` v1.0（path B dogfood 收編 pattern 第一次完整 LIVE 實證、含 §3 Claude 特化 2 row：Over-Conservative Self-Rule + dual-mode context cross-mode forgetting）；(b) `roles/pm/gemini-cli.md` v1.7 → v1.8（best-of-breed 收斂升級、§1 加 2 row + §3 加 dual-mode forgetting row + 新加 §7 vendor 接入回顧獨立段含 signal #5 + #55 LIVE 反例自報）；(c) `roles/pm/_spec.md §7` 對應 AI 表 update。PM 角色 vendor coverage 從 Gemini 單一擴為 Gemini + Claude 雙 vendor。**dogfood signal #59 LIVE 條款化候選**（first-mover baseline 假設失效）+ **SSS S2.4 LIVE 第 1 次完整實證**（跨 vendor 知識聚合 + 互為養分 + 收斂 best-of-breed）|
...
| **v0.10.4 Vendor 介紹 charter 工具紀律 charter common 化（2026-05-06、user LIVE 補充「我看了會想用但我不知道該怎麼安裝、所以去查文件」直接條款化）** | user 在 v0.10.3 ship 後 LIVE 補充：dbSDK Engineer Claude 介紹 `/checkpoints` 給 PM 時用「**如果您希望將此工具集成到 dbSDK 專案中、我可以使用 install-git-hooks.sh 類似的模式...**」被動描述 + 把 install-git-hooks.sh 跟 checkpoints 安裝混淆 — user LIVE 反應「**我看了會想用、但我不知道該怎麼安裝、所以去查文件**」+ user 直接條款化「**我希望可以直接趕緊 ship、因為我現在就會使用、可以快速驗證**」（同 v0.5.8 / v0.7.1 / v0.7.4 / v0.9.0 user 直接條款化 pattern、不走累積門檻）。**ship 內容**：(a) `core/ai-vendor-onboarding §3.5` 新加段「Vendor 介紹 charter 工具紀律」（charter common 條款化、所有 vendor 繼承）— 三段流程（介紹用法 + 例子 / 主動詢問「要現在幫你裝嗎？」/ user 同意直接跑）+ 反例 #53 LIVE 範本對比 + 配套紀律 §3.5.4「介紹前必先 ReadFile 對應 spec」（#54、#32 family 延伸）；(b) `tools/vendor/commons/README.md` 工具清單段重寫 user-facing onboarding 化；(c) `roles/pm/gemini-cli.md §3.7` v1.5 → v1.6 觸發場景擴展 + 標 §3.5 charter common 紀律的 PM 實作落地。**設計學意義**：v0.7.3 北極星「不讓 user 記」演化軌跡 v0.10.x 系列完整化第 4 段 — 對 **user-facing AI 行為角度**（v0.7.x 採用方角度 / v0.9.0 AI 角度 / v0.10.3 maintainer 角度 / **v0.10.4 user-facing AI 行為角度**）。**SSS S1「user 授權閘」第 6 次 LIVE prototype 累積**（user LIVE 補充觀察 → AI 顯化結構性 framing 三設計選擇 → user 直接拍板 (B) charter common 條款化、要求快速 ship 用以實際使用驗證、繞 (A) 累積觀察門檻）|
...
| **CryptoBot 反向接入完成（2026-05-06、A3 公理反向實證最完整迴圈 + v1.0 公開化最後一片拼圖）** | 接入版本 v0.10.1 + standard preset 22/25 條 + PM Gemini + Engineer Claude 雙 vendor + `/charter-doctor` + `/charter-upgrade-verify` 跑過全綠 + **DISCIPLINE.md / IRON.md 改為引用 charter `core/*`、`management/` 收編進 `agent-commons/`**（NEXT.md §6 完整達成）。**設計學意義**：CryptoBot 是 charter 紀律的**原始萃取源頭**（v0.4.2 templates 1:1 從 CryptoBot capsule / handoff / IM 結構萃取）、現在 charter 抽象層後反向接入回原專案 — A3「專案 ⊥ 框架」公理對「自己的根」反向迴圈完整閉環。**第三個 production 採用案例 + 全 standard preset**：YC_AIAgentCrew (AI agent crew) / 公司 dbSDK (內部 SDK) / CryptoBot (量化交易) 三個截然不同領域全用 standard 22/25 = charter 抽象層成熟度的關鍵實證、**v1.0 公開化前最後一片拼圖到位**。**接入時機 v0.10.1 設計觀察**：在 v0.10.0 commit-hook H1-H6 + v0.10.1 step 0.5 charter version 主動通知 ship 後接入、v0.10.2 H7 F6 schema-driven 攔截 ship 前接入 — CryptoBot 下次 commit 升 v0.10.2+ 時若 profile.yaml 漏 F6 會被 H7 binary 攔（自然觸發弱保證項升結構強制升維體驗、SSS S3 propagate LIVE 觀察候選）|
| **CryptoBot 反向接入後 dogfood signal #55 LIVE 重大發現（2026-05-07、Gemini PM「越矩」根因深化）→ 觸發 v0.10.5 PATCH ship** | **背景**：CryptoBot 反向接入 v0.10.1 standard 後、user LIVE 親自發現 Gemini PM 持續越矩的真主因 — 「**Agents 的 generalist 會一直自動去分包任務、導致我們的卡控流程會被他自己分包任務這一塊偷偷的繞過、如今把它關掉整個框架就順起來了、`/agents disable generalist`**」。**結構觀察**：① **dogfood signal #5 根因深化**（v0.6.0 條款化以為治本「LLM 主動繞路」、實際 **Gemini CLI vendor 預設行為層**才是真主因 — PM 不需「自己想繞」就被自動分包）② **dogfood signal #55 同 family** = #5（v0.6.0、LLM 主動繞路）+ #41（v0.9.10、Kiro vendor fallback 讀 claude-code.md 誤認身份）+ #55（v0.10.5、Gemini vendor 預設 sub-agent 自動分包）三 signal 升 family「**vendor 預設行為層 vs charter 紀律**」結構性張力 ③ **對應 v0.8.2 §設計哲學第 5 條「弱保證項升結構強制」家族新類型**（vendor onboarding 預設行為紀律、既有 3 軸：條款層 / 多 actor 互檢 / commit-hook binary 之外的第 4 軸）④ **user 直接條款化 pattern**（同 v0.5.8 / v0.7.1 / v0.7.4 / v0.9.0 / v0.10.4、累積 1 次跳累積門檻、user 明示 scope「提醒就好」）⑤ **CryptoBot 反向接入即時受益**：v0.10.5 ship 後 CryptoBot 升版即得到 PM init 必提醒紀律。**v0.10.5 ship**：`roles/pm/gemini-cli.md §3.5.5` 新加段（PM init 必提醒採用方執行 `/agents disable generalist`）+ §3.2 派發任務既有 `invoke_agent(agent_name="generalist")` 寫死的設計衝突明示 follow-up（留累積 ≥ 2 次 LIVE 觀察評估重構為 cross-AI handoff）。**SSS S1「user 授權閘」第 7 次 LIVE prototype 累積**（user LIVE 發現 → 明示 scope → AI 顯化 framing 三選項 + §3.2 衝突 → user 拍板 🅱️ 直接條款化 PATCH ship、繞 (A) 純記錄保守 + (C) MINOR 結構升維）|
...
| **v0.8.1** | **dogfood-driven hardening 第十四 / 第十五循環 — multi-perspective sub-agent 反向校準 + SSS S3 起手實證**：multi-perspective 4 sub-agent 並行獨立評估（框架結構師 / 核心理念守護者 / 軟體工程師 / 採用方）+ maintainer 綜合補出 4 個第一輪沒到的深度（雙軸矩陣 / 方向性誤讀 / token 估算挑錯 / Phase 5b 解循環依賴）。三 signal 同 LIVE session 條款化（#24 升工具層 + #16 順手修 + SSS S3 起手）。`tools/doctor-spec.md §3.7-§3.9` 既有 error codes 全加四欄 spec-as-data 結構（合規規定 / 修補方向 + 約束 / 反例）+ §3.10 W901 採用方文檔變更歷史 sync 校驗（dogfood signal #24 升工具層條款化）|
| **v0.8.2** | **dogfood-driven hardening 第十六循環 — 雙軸矩陣 framing 第一段（multi-perspective 結構師金礦落地）**：`README.md §設計哲學` 新加第 5 條「雙軸座標 — 哪些紀律靠誰守」（物理依據軸 + 檢測時點軸 + 依賴 LLM 紀律的條款清單 + 對齊紀律三題）+ 21 條 `core/*.md` 開頭加 blockquote 三新行（保證強度 / 檢測時點 / since）— 對齊「弱保證項升結構強制」設計方向、第二日連續 ship 對齊 v0.7.4 雙軌節奏「頻繁小擴增 PATCH」精神 |
| **v0.9.0** | **dogfood-driven hardening 第十七循環 — 紀律完整性 + AI 自我覺察升維（charter 完成 v0.7.3 北極星雙邊閉環）**：4 條新加 condition（21 → 25 條 / 12 → 13 個架構級概念）：(1) `individual-learning-loop`（第 13 個架構級概念、補完接班場景四軸的第 4 軸、user 明示「框架必備」signal #34）；(2) `diagnose-remediate-protocol`（SSS S3 架構級條款化、v0.8.1 起手實證的終局）；(3) `adoption-lifecycle`（5 階段 lifecycle 完整化）；(4) `condition-mutability`（紀律本體、3-strike + AI 修訂權限分層）。新範本 `reflection.md.tpl` + 新 preset `essential.yaml`（3-5 條 / < 5k init token） + 新 spec `tools/uninstall-spec.md`（保留最後的溫柔精神）+ `init-template §3.3.2` 七步驟 → 八步驟（加 step 0 讀過去違反紀錄）+ `doctor-spec §3.11` 個體學習迴圈合規（W1101/W1102/E1103）+ `post-upgrade-verify-spec` 模式 B/C ship。**9 個 dogfood signal 同 release 條款化**（#11/#26/#27/#28/#30/#31/#32/#33/#34）。**v0.7.3 北極星雙邊閉環完成**（採用方角度 ✅ 既有 + AI 角度 ✅ v0.9.0 補完）+ **SSS S1 啟動前置條件齊備** |
| **v0.9.1-v0.9.10** | **PATCH 系列**：v0.9.1 doctor Gap 偵測 + Doctor 角色概念層（dogfood signal #36）/ v0.9.2 PM init `/checkpoints` 後置介紹 / v0.9.3 checkpoints_handler.sh 自動版本偵測升版引導 / v0.9.4 handler 移至 vendor/commons + 橋接 vs 邏輯層 framing / v0.9.5 commit_save 補 cp（dogfood signal #37）/ v0.9.6 save 後交班詢問 + deactivate_all_active / v0.9.7 reflection.md.tpl frontmatter 修正 / v0.9.8 signal #38 ① ④ ⑥ 修補 / v0.9.9 BOOTSTRAP.md 入口檔（v0.7.6 議程落地、dogfood signal #21 修補）/ v0.9.10 init-template vendor spec 誤讀防呆（signal #41 Kiro fallback 讀 claude-code 誤認身份）|
| **v0.10.0** | **dogfood-driven hardening 第十八循環起手 — commit hook vendor 中立架構 ship + 6 條同源 signal 結構強制升維**：新檔 `tools/commit-hook-spec.md` 定義 H1-H6 校驗紀律（spec-as-data 四欄結構）+ reference 實作 `tools/vendor/commons/charter-commit-checks.sh v1.0`（bash binary）+ 安裝器 `install-git-hooks.sh v1.0`（thin shim 進 .git/hooks/pre-commit）。**6 條同源 signal 一次收編**：#33 不自報 / #35 自激活 / #42 雙寫漏 / #43 檔名漂浮 / #44 sprint 混 reflection / #45 致 XXX 缺。**vendor 中立性實證** vs 原版「.claude/hooks/ / .gemini/hooks/ 各 vendor 私有」設計、走 git 原生 + agent-commons 共用 script — Claude/Gemini/Kiro/Cursor/人類 commit 全攔。**雙軸軸 1「結構強制」首次大規模落地** |
| **v0.10.1** | **PATCH — Charter version 主動通知 + signal #47 條款化 + 首次第三方 LIVE 工作實證**：`init-template §3.3.2` 加 step 0.5（charter version 比對 + 主動通知）— AI 在 step 0 後讀 framework 版本 vs project profile.yaml.charter_version、三分支處理（equal 跳過 / framework > project INFO / framework < project ERROR）。設計參考 prior art：v0.9.3 checkpoints_handler 三分支版本偵測 propagate 到 charter version 本身。**v0.10.1 ship 不到 24 小時內首次第三方 LIVE 實證**（公司專案 dbSDK Engineer Claude 自然執行 step 0.5 + 致 PM directive header 同 session 雙重首次工作）|
| **v0.10.2** | **BREAKING-LITE PATCH — 加 H7 schema-driven 強制必啟集合 binary 攔截**：對應 dogfood signal 累積爆量（#46 ≥3 次 / #31 ≥5 次同類同 session / #52 候選 三層雙重防禦對 F6 整體 LIVE 失效、user 直接條款化）。新檔 `tools/profiles/_required.yaml`（schema source of truth、強制必啟集合定義、當前 ship REQ-001-F6）+ `charter-commit-checks.sh v1.0 → v1.1`（加 check_h7 + h7_check_f6_enabled）+ `commit-hook-spec.md` H7 段（spec-as-data 四欄結構）。**4 案例 LIVE 測試全綠**（inline no-F6 / inline with-F6 / multi-line no-F6 / F6-in-comment 反 false-pass）。**未來擴展紀律**：F7 / F8 加進來 → 改 `_required.yaml` 加 entry + 補對應 hook inline check（5-10 行 bash）→ 採用方 git pull 即傳播、不需再加 H8/H9 binary。標 BREAKING-LITE 對齊 user 紀律「有衝突就代表沒有向下兼容」（v0.7.0 mislabel 教訓） |
| **v0.10.3** | **PATCH — SSS S3 propagate 終局 + maintainer-only lint binary**：對應 user LIVE 提問「升版這些檢查可否自動納入新設計、不然每次都很容易漏掉」+ dogfood signal #46 #31 終局議程。`post-upgrade-verify-spec.md §3.1-§3.5` 全 5 軸 22 個 check item 升 spec-as-data 四欄結構（~660 行加固）+ `doctor-spec.md §3` 段首加全局「真實 stdout 證據要求」紀律段 + `README.md §設計哲學` 第 5 條雙軸座標加「**升維軌跡 — 結構強制升維紀錄**」段（雙軸 framing 第三段、列 charter v0.5.7 起 11 個 dogfood-driven hardening 循環的弱保證項升維 trajectory）+ 新檔 `tools/charter-spec-lint.sh v1.0`（maintainer-only binary、L1 tool spec 四欄結構 + L2 core/*.md 雙軸 blockquote 偵測、跑全 charter 0 errors / 0 warnings）+ `maintainer-discipline §3.1.1` 加 lint binary 紀律段。**對齊 v0.7.3 北極星「不讓 user 記」延伸到「不讓 maintainer 記」**（規範自動化的元層落地）|
| **v0.10.4** | **PATCH — Vendor 介紹 charter 工具紀律 charter common 化**（v0.10.x 系列收尾、規範自動化元層第 4 段）：對應 user LIVE「我看了會想用、但我不知道該怎麼安裝、所以去查文件」+ user 直接條款化「我希望可以直接趕緊 ship、因為我現在就會使用、可以快速驗證」（同 v0.5.8 / v0.7.1 / v0.7.4 / v0.9.0 user 直接條款化 pattern、不走累積門檻）。`core/ai-vendor-onboarding §3.5` 新加段「Vendor 介紹 charter 工具紀律」（charter common 條款化、所有 vendor 繼承）— 三段流程（介紹用法 + 例子 / 主動詢問「要現在幫你裝嗎？」/ user 同意直接跑）+ 反例 dogfood signal #53 LIVE 範本（dbSDK Engineer Claude「如果您希望」被動描述 vs 主動詢問版本對比）+ 配套紀律 §3.5.4「介紹前必先 ReadFile 對應 spec」（dogfood signal #54、#32 family 延伸）+ `tools/vendor/commons/README.md` 工具清單段 user-facing onboarding 化（白話 + 例子 + 一鍵 prompt）+ `roles/pm/gemini-cli.md §3.7` v1.5 → v1.6 觸發場景擴展。**v0.7.3 北極星「不讓 user 記」演化軌跡 v0.10.x 完整化第 4 段** — 對 user-facing AI 行為角度（v0.7.x 採用方角度 / v0.9.0 AI 角度 / v0.10.3 maintainer 角度 / **v0.10.4 user-facing AI 行為角度**）|

### v0.10.x 系列 — 規範自動化元層四角度閉環（dogfood-driven hardening 第十八循環完整收編）

| 對「不讓 X 記」的 X | 落地 | Ship |
|---|---|---|
| 採用方 | walkthrough + verify + spec-as-data + BOOTSTRAP | v0.7.x |
| AI 個體 | individual-learning-loop + init step 0 強制讀 + reflection 雙寫紀律 | v0.9.0 |
| AI 行為層 | commit-hook H1-H7 binary 攔截（H7 schema-driven） | v0.10.0 + v0.10.2 |
| maintainer | lint binary + spec 段首全局紀律 + 升維軌跡表 | v0.10.3 |
| **user-facing AI 行為** | **vendor 介紹 charter 工具紀律（§3.5 charter common）** | **v0.10.4** |

→ charter v0.7.3 北極星「**不讓 user 記**」**完整 360 度閉環**（5 個維度全 cover）。**SSS S1「user 授權閘」累積 ≥ 6 次 LIVE prototype**（v0.10.0 ~ v0.10.4 連續 ship、user 不深入 design 細節、AI 顯化結構性 framing、user 拍板 ship 路徑）。
