...
> **更新時間**：2026-05-23（**v0.13.0 ship — Engineer × Codex Desktop vendor 接入完成 MINOR**）— 依 `core/ai-vendor-onboarding §3` 邀請制四步驟走完（charter 寫概念層 → 邀請 Codex 寫 vendor 層 → Claude Code 既有 vendor 校正 regression 無 regression → maintainer 三層簽收）+ v0.12.0 canonical / adapter pattern 首次新 vendor 接入 LIVE 驗證、Codex 一輪即過 step 4 簽收（anti-`dogfood signal #41` 教科書級實證：拒絕宣稱 `.codex/commands/` 不存在的 native slash command、改用 Codex Skill schema）+ Engineer 角色 vendor coverage 從 1 → 2（Claude Code + Codex Desktop）+ 新 dogfood signal **#62 候選登記**（global skill version drift、Codex `~/.codex/skills/` user-global 共用、#58 vendor skill abstraction family 延伸、累積觀察）+ Codex adapter Step 3 cross-platform sweep follow-up（#48 family）
> **前次更新**：2026-05-22（**v0.12.0 ship — 3 in 1 BREAKING-MEDIUM** — (1) Antigravity vendor 接入完成 + (2) **Canonical Init Spec Layer ship**（SSS S2.5、架構級概念第 15 個、dogfood signal #61 條款化）+ (3) **agents-commons rename**（BREAKING-MEDIUM、`agent-commons/` → `agents-commons/`、架構級概念第 14 個 `core/vendor-lifecycle.md` 收編 #5/#41/#55/#58/#59/#60 family）。架構級概念 13 → 15、條款數 25 → 27）
> **前次更新**：2026-05-22（**v0.11.0 ship** — Gemini CLI 棄用觸發 Antigravity CLI vendor path A AI-DRAFTED-FROM-GEMINI-V1.8 接入 + 同日 LIVE 第二輪校正 + Gemini CLI 標 LEGACY_AUTO_IMPORTED、dogfood signal #60 LIVE 條款化候選登記、A1 公理時運實證）
> **前次更新**：2026-05-21（**v0.10.6 ship** — Claude PM v1.0 接入 + Gemini PM v1.8 best-of-breed 收斂升級、path B dogfood 收編 pattern 第一次完整 LIVE 實證、dogfood signal #59 條款化候選登記）
> **前前次更新**：2026-05-20（dogfood signal #58 候選新增 — Claude vendor skill abstraction 紀律對齊不完整、v0.9.3 Gemini handler mapping.yaml 抽象化升維未 propagate Claude 端 / CryptoBot 跨軌污染 LIVE）
...
charter 既有設計把 `core/violation-reflection §2`「LLM 不可矯正、價值在集體記憶」精神實作為**集體記憶層**（`state/failure_mode_log.md` + `institutional-memory/`）— 但**個體記憶層 + 學習迴圈紀律缺失**。

| 應該有 | charter 實際有 | 完整度 |
|---|---|---|
| 集體記憶（跨事件 F-mode 累積）| ✅ `state/failure_mode_log.md` + `institutional-memory/` | 完整 |
| 個體記憶位置（每角色 / 每 vendor 個人反省）| ⚠️ `agent-commons/roles/<role>/reflections/` 目錄存在（init scaffold）| 位置具備 |
| 個體 reflection 範本 | ❌ `templates/agent-commons/reflection.md.tpl` **不存在**（v0.4.2 6 份 templates 沒含 reflection）| 漏 |
| 寫入紀律（命中 F-mode 必雙寫到個人 reflections + 集體 log）| ⚠️ `core/violation-reflection §1.4` 模糊 — 沒明示「個人 vs 集體雙寫」| 模糊 |
| 讀取紀律（每次 init / 接班時必讀過去 reflections）| ❌ `core/init-template §3.3.2` 七步驟**沒「step 0 讀過去違反紀錄」**| 漏（最關鍵）|
| 跨 session 學習迴圈 | ❌ 沒條款 | 漏 |

→ 學習迴圈三角缺兩角：**寫位置有 / 寫紀律模糊 / 讀紀律完全沒**。

**第四個結構性盲區（接班場景三軸 → 四軸）**：

charter v0.5.7 working-stack-discipline 補完接班場景三軸：
1. session 末邏輯結案（`handoff-chain`）
2. AI 廠商換手（`cross-ai-handoff`）
3. session 內物理中斷再續（`working-stack-discipline`）

**第 4 軸（漏）**：個體 AI 跨任務 / 跨 session 學習迴圈

→ 與前 3 軸正交、是 v0.9+ 必補的結構性盲區。

**加固方向（v0.9+ 設計素材）**：

新加 condition **`core/individual-learning-loop.md`**（候選第 22 條 core 條款、第 13 個架構級概念）：

1. **寫入紀律**：命中 F-mode 後、補 violation-reflection **雙寫**：
   - 集體層 `state/failure_mode_log.md`（既有）
   - **個體層 `roles/<role>/reflections/<YYYY-MM-DD>_<f-mode>_<short>.md`**（新加紀律）
2. **讀取紀律**：擴 `core/init-template §3.3.2` 七步驟為**八步驟**：
   - 加 **step 0「讀過去違反紀錄」** — 每次 self-instantiation 必先 read 自己 reflections/ + failure_mode_log + IM
   - **跨 session 接班 AI 同樣強制**（依 cross-ai-handoff）
3. **範本**：新加 `templates/agent-commons/reflection.md.tpl`（依 charter 既有 6 份 templates pattern、scaffold 含「命中模式 / 學習要點 / 對應條款」三段）
4. **校驗**：擴 `tools/doctor-spec.md` 加 §3.11「個體學習迴圈合規」校驗（每命中 F-mode 是否有對應 reflection 個體層 entry）

**雙軸矩陣對應**（multi-perspective 結構師金礦）：

| 物理依據 | 檢測時點 | 現況 | 升維後 |
|---|---|---|---|
| 單 actor 自律 | runtime（命中後自報）| `violation-reflection`（弱）| 維持弱（價值在 audit trail）|
| 結構強制 | post-task / pre-init | ❌ 沒有 | **新加 individual-learning-loop（強）**：reflections/ 寫 + init 讀強制 |

→ multi-perspective 結構師金礦「弱保證項升結構強制」最對齊的議程。

**對 SSS S1「AI 自治協作 + user 授權閘」前置基礎建設**：

SSS S1 設計能 work 的前提是「**AI 不再犯**」 — 沒有 individual-learning-loop（#34）+ failure-mode 自報結構強制（#33）兩個 signal 升維、AI 自治 = AI freelance（重蹈覆轍）。

**v0.9+ 議程位階重排**：

| 原優先 | 新優先 | 議程 |
|---|---|---|
| ① | ① | **`core/individual-learning-loop.md`**（**signal #34 條款化、最深 + 最關鍵 + user 明示框架必備**）|
| ② | ② | SSS S3「引導式紀律」架構級條款化（`core/diagnose-remediate-protocol.md`）|
| ③ | ③ | `core/adoption-lifecycle.md` |
| ④ | ④ | `core/condition-mutability.md` |
| ⑤ | ⑤ | essential preset (3-5 條 core)|

→ #34 升至 v0.9+ 議程**第一順位** — charter 紀律完整性的最後一塊拼圖、SSS S1 啟動前置條件。

**LIVE 實證觸發脈絡**（2026-04-30 公司專案 NO3._dbSDK_Imporve）：

Gemini PM 連續違反 F1/F3/F5/F6 + role-separation §3.5 多次（共 ≥10 次 F-mode 命中、見 signal #33 詳細列表）— **但沒任何主動補交 violation-reflection**。user 親手抓 + 親手命令 Gemini 補 failure_mode_log + _role.md Status 升級 + reflection 個體層 entry。**user 抓核心痛點**：「**他要把這個列入他自己的檢討清單、做為以後不會再犯的依據**」 — charter 紀律完整、但執行端 AI 沒做、且**做了集體層 log 也不會「下次不犯」、必須個體層 reflections 雙寫 + 八步驟強制讀**。

→ user 明示「**框架必備**」 — 不走 ≥3 次累積門檻、直接列 v0.9+ 議程第一順位（同 v0.5.8 / v0.7.1 / v0.7.4 user 直接條款化 pattern）。
...
- **dogfood signal #38 — reflection 路徑 / 格式合規問題：雙 AI 雙樣本 LIVE 實證（2026-05-01 dbSDK 專案）**【⚠️ 部分修補 v0.9.7（②③⑤）+ v0.9.8（④⑥ 修補、① 觀察中）；① 累積至 ≥3 次後評估 doctor W1101 路徑 binary check】
  ...
  **Layer 1 — Gemini PM：路徑自創（最嚴重）**
  寫入 `.gemini/self_audit/management_rules.md`（自創概念）+ 引用不存在的 `[GLOBAL_TERRITORIAL_HOOK]`。
  `.gemini/` 是 vendor 橋接層（slash command 目錄），非 charter common memory root。
  **根因**：Gemini 混淆「vendor 私有目錄 vs charter agent-commons/」+ 同 signal #32「LLM 不查 templates 就自編格式」同源。

  **Layer 2 — Claude Engineer：路徑正確、發現框架缺口（設計問題）**
  Frontmatter 五欄 ✅、Doctor §3.11 W1101/E1103 能過 ✅。但：
...
- **✅ 新 dogfood signal #59 條款化候選登記 — `core/ai-vendor-onboarding §3 step 3` 原設計隱含「first-mover baseline」假設、第二個 vendor 優於第一個時失效（2026-05-21 LIVE 實證、v0.10.6 path B dogfood 收編 LIVE）**【累積 1 次 LIVE、user 直接條款化 pattern；候選修法：v0.12.0 MINOR 條款演化「雙向 best-of-breed 收斂」或新加 `core/cross-vendor-best-of-breed.md`（架構級概念第 14 個候選）】

- **✅ 新 dogfood signal #60 LIVE 條款化候選登記 — Gemini CLI 棄用事件（Google 2026-05-19 宣布 Antigravity CLI 取代、2026-06-18 對 Pro/Ultra/free 斷線）觸發 charter v0.11.0 Antigravity vendor path A AI-DRAFTED 接入 + 同日 LIVE 第二輪校正（2026-05-22 LIVE 實證）**【累積 1 次 LIVE、user 直接條款化 pattern；v0.11.0 ship 立刻 hooks：(1) `roles/pm/antigravity-cli.md` v1.1 path A AI-DRAFTED-FROM-GEMINI-V1.8 + 同日 LIVE 第二輪校正（Skills 子目錄 `~/.gemini/skills/<name>/SKILL.md` + frontmatter `name:` 必填、由 dbSDK PM AI 親讀 SKILL.md 實檔回報觸發）/ (2) `roles/pm/gemini-cli.md` v1.9 標 LEGACY_AUTO_IMPORTED + §9 採用方遷移指引 / (3) `examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md` walkthrough / (4) UPGRADE.md 加新 row + 顯著警告段；**候選升 core 條款層** v0.12.0：family（#5 vendor 主動繞路 + #41 vendor fallback 誤讀身份 + #55 generalist 自動分包 + #58 Claude vendor skill abstraction 對齊 + #59 first-mover baseline 假設失效 + #60 vendor 自己被棄用）→ 評估新增 `core/vendor-lifecycle.md`（架構級概念第 14 個）三場景涵蓋；**A1 公理時運實證**：v0.10.6 ship Claude PM v1.0 後 < 24 小時 Gemini 棄用消息抵達、即使 Gemini 消失 PM 角色不死（path B dogfood 收編回頭看 = vendor 換手保險）；**vendor 中立架構反向實證**：v0.10.0 commit-hook git-native + v0.9.4 handler `commons/` 兩個設計選擇 survive vendor death = `dogfood signal #41 vendor lock-in 反思` 終局實證；**多 perspective 第十四循環新類型**：採用方 vendor AI 對 charter 行使結構性反向觀察（dbSDK PM AI 報告全文存 `.claude_temp/DBSDK-CROSS-VENDOR-SLASH-COMMAND-REPORT-2026-05-22.md`、含 §4 SSS S2.5「Canonical Init Spec Layer」議程提案、留 v0.12.0 評估 ship）】

- **🚀 SSS S2.5 候選新議程 — Canonical Init Spec Layer（dbSDK PM AI 報告 §4 提案、2026-05-22 LIVE 觸發）**：dbSDK PM AI 跨 vendor 報告揭露三 vendor pm-init 量化差異（604 / 11842 / 1645 bytes）+ 缺 canonical init spec layer 缺口。候選方案：新加 `core/init-spec-schema.md`（vendor-neutral schema、架構級概念第 14 個候選之一）+ `roles/<role>/init-spec.md` canonical + `templates/vendor-adapters/*.tpl`（3 個 vendor adapter）。**留 v0.12.0 MINOR 評估 ship**、合併 agents-commons rename（v0.12.0 BREAKING-MEDIUM、user 2026-05-22 LIVE 提出）+ #60 family core 條款層升維 = **v0.12.0 3 in 1 ship**（vendor lifecycle / canonical init spec / agents-commons rename）。
...
- **新 dogfood signal #39 候選 — BOOTSTRAP.md「第一次接入」prompt 未處理「既有協議遷移」場景（2026-05-03 CryptoBot 接入 LIVE）**【累積 1 次；候選修法：BOOTSTRAP.md 加「既有協議」分支 prompt】
  ...
  **兩個具體問題**：
  1. **maintainer 建議「路徑 A（自己寫，參考 IRON.md）」** — 等於讓 user 自己整理鐵律格式、AI 只負責跑格式；user 要的是「路徑 B（AI 讀既有協議重新整理，user 只校正）」。根因：BOOTSTRAP.md prompt 沒有提示「若專案已有既有協議 → 選路徑 B + 告知 AI 既有協議路徑」的引導邏輯。
  2. **maintainer 建議「Step 1 跳過」** — 混淆了「user 不需要手動跳過 / 讓 AI 自行 confirm framework 存在」；user 應從 Step 1 完整跑（AI 確認存在才繼續，不存在才 clone）。根因：BOOTSTRAP.md prompt 已正確寫「請先確認目錄存在，若不存在請執行 git clone」，但 maintainer 的引導讓 user 以為要改 prompt 來跳過。

  **根因**：BOOTSTRAP.md「🆕 第一次接入」prompt 設計為「空白專案起手」場景，沒有「既有協議遷移」分支。domain-axioms 路徑 B 的觸發範本在 `templates/agent-commons/domain-axioms-via-ai-draft-prompt.md.tpl` 裡，但 BOOTSTRAP.md prompt 沒提示 user 何時該選路徑 B、也沒提示「路徑 B 時要告知 AI 既有協議路徑」。

  **候選修法**（v0.9.x PATCH）：`BOOTSTRAP.md §🆕 第一次接入` prompt 加一個「**有既有協議 / 既有文件想遷移**」變體或 note：「若專案已有既有協議文件，domain-axioms 選路徑 B，並在 prompt 中補：`特別注意：<路徑> 是既有協議，請作為主要來源之一讀入，重新整理成 AgentCharter domain-axioms 格式`」。

  **累積**：1 次（2026-05-03 CryptoBot LIVE）。**判斷**：BOOTSTRAP.md 是剛 ship 的 v0.9.9 新檔，此缺口屬「blank slate 場景設計、遺漏 migration 場景」設計缺陷；累積至 2-3 次或遇第二個接入案例時 PATCH 修法。
...
✅ **dogfood-driven hardening 第十六循環**（multi-perspective 第十四循環結構師金礦落地、第二日連續 ship）：

✅ **新增**：
- `README.md §設計哲學` 新加第 5 條「雙軸座標 — 哪些紀律靠誰守」 — 物理依據軸（強制 / 多 actor 互檢 / 單 actor 自律）+ 檢測時點軸（init / runtime / post-upgrade / handoff）+ 依賴 LLM 紀律的條款清單（弱保證項公開）+ 對齊「對未來修訂的紀律」三題新增雙軸對齊軸
- 21 條 `core/*.md` 開頭加 blockquote 三新行（保證強度 / 檢測時點 / since）— 採用方執行邏輯零影響、純文檔層擴增

✅ **連動更新**：三 preset yaml `0.8.1` → `0.8.2` + ADOPTION/TUTORIAL/maintainer-load 升版號 + ADOPTION §13 v1.10 + TUTORIAL 變更歷史 v1.10 + CHANGELOG v0.8.2 段 + STATUS Version 軌跡 v0.8.2 row + STATUS frontmatter「當前版本」+ Git tags

✅ **議程位階**：multi-perspective 第十四循環結構師金礦落地 — 雙軸正交矩陣（物理依據 ⊥ 檢測時點）顯化進 charter 條款體系；對齊採用方金礦「essential preset 才是真槓桿」+ 「依賴 LLM 紀律的條款清單」採用方知情權 + 理念守護者金礦「LLM 不可矯正方向性誤讀」設計方向

✅ **嚴守向下兼容**：純擴增 spec 層 + 文檔層、採用方升版只改 `charter_version` 一行；零執行邏輯影響

✅ **後續議程**：v0.8.3 PATCH = 雙軸矩陣 framing 第二段（21 條條款 §X 細節 inline marker）+ 第三段（lint binary 派生「依賴 LLM 紀律的條款清單」）+ SSS S3 propagate 到 post-upgrade-verify-spec / init-spec
