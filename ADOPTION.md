...
- **v1.22（2026-05-22，charter v0.11.0）** — 🔥 **Gemini CLI 棄用觸發 Antigravity CLI vendor 接入 MINOR 連動 sync**（dogfood signal #60 LIVE 條款化候選、Google 2026-05-19 宣布 Gemini CLI 2026-06-18 對 Pro/Ultra/free 斷線）。**對採用方影響**：(a) 使用 Gemini CLI **免費 / Pro / Ultra tier** 採用方 — **6/18 前必須遷移 Antigravity CLI**、依 `examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md` walkthrough（10 步流程 + 6 常見問題）；(b) **Enterprise / 付費 API key / OSS** tier — 不必動、Gemini CLI vendor spec 仍 ACTIVE；(c) 不用 Gemini CLI（只用 Claude Code 等）— 完全不受影響。**升版內容**：(a) 新檔 `roles/pm/antigravity-cli.md` v1.1（path A AI-DRAFTED-FROM-GEMINI-V1.8 + 同日 LIVE 第二輪校正、Skills 子目錄結構 `~/.gemini/skills/<name>/SKILL.md` + frontmatter `name:` 必填）；(b) `roles/pm/gemini-cli.md` v1.8 → v1.9（vendor_status 升 LEGACY_AUTO_IMPORTED + 新加 §9 採用方遷移指引、既有 §1〜§8 不刪）；(c) `roles/pm/_spec.md §7` 對應 AI 表 update（加 Antigravity row、Gemini 標 LEGACY_AUTO_IMPORTED）；(d) `UPGRADE.md` 加新 row + 顯著警告段；(e) 4 個 preset YAML charter_version `0.10.6` → `0.11.0`。**LIVE 校正紀錄**：v1.0 ship 同日 dbSDK PM AI 親讀 SKILL.md 實檔回報兩處 path A 預估錯誤 → maintainer 立刻校正 §3.6 Skills 路徑 + frontmatter `name:` 必填、升 v1.1。**設計學意義**：**A1「角色 ⊥ AI」公理時運實證** — charter v0.10.6 ship Claude PM v1.0 後不到 24 小時 Gemini 棄用消息抵達、即使 Gemini 消失 PM 角色不死（path B dogfood 收編 = vendor 換手保險）+ **vendor 中立架構反向實證**（v0.10.0 commit-hook + v0.9.4 handler `tools/vendor/commons/` survive vendor death、dogfood signal #41「vendor lock-in 反思」終局實證）+ `core/ai-vendor-onboarding §3 step 2` 邀請制 path A LIVE 第二次應用實證。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md`](./examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md)**。詳見 CHANGELOG v0.11.0 段。

- **v1.21（2026-05-21，charter v0.10.6）** — Claude PM v1.0 接入 + Gemini PM v1.8 best-of-breed 收斂升級 PATCH 連動 sync。**零採用方動作要求**（純 vendor spec 新增 + 既有 vendor spec 擴增）— 升版只改 profile.yaml `charter_version: "0.10.5"` → `"0.10.6"`、無其他動作。**升版內容**：(a) 新檔 `roles/pm/claude-code.md` v1.0（481 行、Claude Code 端 PM vendor 實作、path B dogfood 收編 pattern 第一次完整 LIVE 實證）；(b) `roles/pm/gemini-cli.md` v1.7 → v1.8（best-of-breed 收斂升級：§1 加 2 row + §3 加 dual-mode forgetting row + 新加 §7 vendor 接入回顧獨立段 + §7 變更歷史升 §8）；(c) `roles/pm/_spec.md §7` 對應 AI 表 update（Claude Code ✅ v1.0 / Gemini CLI ✅ v1.8）。**對採用方影響**：PM 角色 vendor coverage 從 Gemini 單一擴展為 Gemini + Claude 雙 vendor — 採用方接入 PM 角色時可選 Gemini CLI 或 Claude Code 任一 vendor。**設計動機**：dogfood signal #59 LIVE 條款化候選 — `core/ai-vendor-onboarding §3 step 3` 原設計隱含「first-mover baseline」假設、第二個 vendor 優於第一個時失效；user explicit 授權走 maintainer 代修反向 regression。**對應 v0.7.3 北極星「培養魚塘、不討魚」精神 + SSS S2.4 候選議程「跨 vendor 知識聚合 + 互為養分 + 收斂 best-of-breed」LIVE 第 1 次完整實證**。詳見 CHANGELOG v0.10.6 段。
- **v1.20（2026-05-10，charter v0.10.5）** — Gemini CLI 預設 `generalist` 自動分包處置 PATCH 連動 sync。**零採用方動作要求**（純 vendor spec 擴增）— 升版只改 profile.yaml `charter_version: "0.10.4"` → `"0.10.5"`、無其他動作。**升版內容**：`roles/pm/gemini-cli.md §3.5.5`（v1.6 → v1.7）新加段「Gemini CLI 預設 `generalist` 自動分包處置 — PM init 必提醒」 — PM self-instantiation 開始時必對採用方主動發 reminder（執行 `/agents disable generalist` slash command 對齊 `role-separation §3.5` 卡控紀律）。**對採用方影響**：Gemini PM 接入時會主動建議 user 執行 disable 命令、避免 generalist 自動分包繞 PM 卡控（CryptoBot 反向接入 LIVE 實證）。**設計動機**：dogfood signal #55（user 直接條款化）— Gemini CLI vendor 預設行為層才是真主因、PM 不需「自己想繞」就被自動分包；對應 dogfood signal #5 根因深化框架（vendor 預設行為層 vs LLM 主動繞路）+ v0.8.2 §設計哲學第 5 條「弱保證項升結構強制」家族新類型。詳見 CHANGELOG v0.10.5 段。
- **v1.19（2026-05-06，charter v0.10.4）** — Vendor 介紹 charter 工具紀律 charter common 化 PATCH 連動 sync。**零採用方動作要求**（純條款層擴增）— 升版只改 profile.yaml `charter_version: "0.10.3"` → `"0.10.4"`。**升版內容**：(a) `core/ai-vendor-onboarding.md §3.5` 新加段「Vendor 介紹 charter 工具紀律」（charter common 條款化、所有 vendor 繼承）— 三段流程（介紹用法 + 例子 / 主動詢問「要現在幫你裝嗎？」/ user 同意直接跑安裝）+ 反例 dogfood signal #53 LIVE 範本（被動「如果您希望」描述 vs 主動詢問版本對比）+ 配套紀律 §3.5.4「介紹前必先 ReadFile 對應 spec」（dogfood signal #54、#32 family 延伸）；(b) `tools/vendor/commons/README.md` 工具清單段重寫 user-facing onboarding 化（白話 + 例子 + 一鍵安裝 prompt + 安裝後 LIVE 驗證範例）；(c) `roles/pm/gemini-cli.md §3.7` v1.5 → v1.6（觸發場景擴展 + 標為 §3.5 charter common 紀律的 PM-specific 實作落地）。**設計動機**：對應 user 2026-05-06 LIVE 提問「**我看了會想用、但我不知道該怎麼安裝、所以去查文件**」+ user 直接條款化「**我希望可以直接趕緊 ship、因為我現在就會使用、可以快速驗證**」。**對應 v0.7.3 北極星「不讓 user 記」延伸到「不讓 user 為了用一個工具還要查 charter 文件」**。詳見 CHANGELOG v0.10.4 段。
