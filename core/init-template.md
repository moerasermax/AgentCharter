...
> **保證強度**：結構強制（核心 step 0 過去違反紀錄讀取 + step 5 doctor schema 驗證；step 7 self-instantiation 自驅屬單 actor 自律輔助）
> **檢測時點**：init
> **since**：v0.5.0（v0.5.10 加 step 5 / v0.7.0 加 step 6 PROVISIONAL/ACTIVE / **v0.9.0 加 step 0 讀過去違反紀錄**）
...
> **v0.9.0 起八步驟**（原七步驟 v0.5.0 起、step 5 v0.5.10 加、step 6 v0.7.0 加 PROVISIONAL/ACTIVE 紀律、**step 0 v0.9.0 加讀過去違反紀錄** / **step 0.5 v0.10.1 加 charter version 比對 + 主動通知**）。

```
0. 讀過去違反紀錄（v0.9.0 加；對齊 core/individual-learning-loop §3 讀紀律 — 跨 session 學習迴圈強制起手）：
   - ReadFile <common-memory-root>/roles/<role>/reflections/*.md（個體層、依檔名日期前綴撈最近 5 個；無檔則記錄「首次接班、無歷史」）
   - ReadFile <common-memory-root>/state/failure_mode_log.md（集體層、F-mode 累積）
   - ReadFile <common-memory-root>/institutional-memory/*.md（IM 層、相關五段格式 entry — 與當前角色 / 任務脈絡相關者必讀）
   - 不通則 self-instantiation 視為失敗：step 0 必過才能進 step 1（讀檔 IO 失敗 ≠ 「無歷史」、必區分）
   - 對應 dogfood signal #34（user 2026-04-30 LIVE 公司專案接入抓到「個體學習迴圈紀律缺失」、user 明示「框架必備」直接條款化）
   - 違反 → 視同 F1（假宣告就位 — 未讀過去違反即進入工作 = 跨 session 同盲點復發風險未攔）

0.5. Charter version 比對 + 主動通知（v0.10.1 加；對齊 BOOTSTRAP.md 三層保證 + v0.7.3 北極星「不讓 user 記」延伸到「不讓 user 想到去比對版本」）：
   - 讀 framework 當前版本：head -20 $CHARTER_DIR/CHANGELOG.md、取第一個 `## [X.Y.Z]` 行
   - 讀專案宣告版本：yq '.charter_version' <common-memory-root>/_config/profile.yaml
   - 比對三分支：
     a) **framework_version == project_version** → 跳過、繼續 step 1（最常見路徑）
     b) **framework_version > project_version**（合法、framework 領先專案）→ INFO 提示 user：
        「framework 已升 v<X>、本專案在 v<Y>。
         是否升版？升版指引：$CHARTER_DIR/UPGRADE.md
         （我不會自動改 profile.yaml、等你 explicit 授權）」
        然後繼續 step 1（用專案宣告版本的 spec 工作、避免 spec drift）
     c) **framework_version < project_version**（異常、專案宣告版本 framework 沒有）→ ERROR：
        「專案宣告 v<Y>、但 framework 只有 v<X>、跨 charter 版本一致性違反。
         請先 cd $CHARTER_DIR && git pull」
        中止 init、要求 user 升 framework
   - 對應 dogfood signal #47（charter 主動通知缺位、2026-05-06 user LIVE 觀察「為什麼 git pull 後專案沒升、AI 沒主動提醒」）
   - 設計參考 prior art：`roles/pm/gemini-cli.md §3.7 Step 1`（v0.9.3 加）對 `checkpoints_handler.sh` 的三分支版本偵測（MISSING / STALE / CURRENT），本步驟把同 pattern propagate 到 charter version 本身
   - 違反 → 視同 F1（假宣告就位 — 未做版本比對即進入工作 = 可能用錯版本 spec / 漏看升版可能性）
...
| **跳過 step 0 讀過去違反紀錄即進入 step 1（v0.9.0 加）** | 視同 F1（假宣告就位）— 未讀過去違反 = 跨 session 同盲點復發風險未攔。對應 `core/individual-learning-loop §3` 讀紀律 + 第 4 軸接班場景補完。處置：抽驗方有權**回退 _role.md PROVISIONAL → 重做 step 0**，並要求 AI 列出 reflections/ 最近 5 個 + failure_mode_log + 相關 IM entry 摘要 |
...
| `individual-learning-loop.md`（v0.9.0 加）| §3.3.2 step 0「讀過去違反紀錄」是該條款 §3 讀紀律的**執行載體**；step 0 失敗 → self-instantiation 整體失敗。第 4 軸接班場景（個體 AI 跨任務 / 跨 session 學習）補完。|
...
**動作**：
1. §3.3.2 self-instantiation 從**七步驟擴為八步驟** — 在原 step 1 之前**加 step 0「讀過去違反紀錄」**（讀 reflections/ 最近 5 個 + failure_mode_log + IM 相關 entry；不通則 self-instantiation 視為失敗、step 0 必過才能進 step 1）
2. §3.3.5 違反處置表加一行：跳過 step 0 即進入 step 1 = F1（跨 session 同盲點復發風險未攔）
3. §8 跨條款關係表加 `individual-learning-loop.md`（本條款 §3.3.2 step 0 是其 §3 讀紀律執行載體）
4. frontmatter 保證強度欄補「step 0 過去違反紀錄讀取」+ since 行加 v0.9.0 加 step 0 註記

**觸發**：dogfood signal #34（user 2026-04-30 LIVE 公司專案接入抓到「個體學習迴圈紀律缺失」、user 明示「框架必備」直接條款化、跳過 ≥3 次累積門檻、同 v0.5.8 / v0.7.1 / v0.7.4 user 直接條款化 pattern）。

對應 charter v0.5.7 working-stack-discipline 補完接班場景三軸（handoff-chain / cross-ai-handoff / working-stack-discipline）— **個體 AI 跨任務 / 跨 session 學習迴圈是第 4 軸補完**。

**修訂類型**：MINOR — 加新步驟（step 0）、行為向後相容；既有採用方下次 self-instantiation 時補跑 step 0（既有簽名不溯及）。新採用方 reflections/ 目錄初始空、step 0 記錄「首次接班、無歷史」即可通過、不阻擋首次 init。

**連動範圍**（依 `maintainer-discipline §2.2`）：
- `core/individual-learning-loop.md`（v0.9.0 新加條款本體；本條款 §3.3.2 step 0 是其 §3 讀紀律執行載體）
- `tools/doctor-spec.md §3.11`（v0.9.0 加；reflections/ 累積 + 雙寫對應校驗 — W1101/W1102/E1103）
- `templates/agent-commons/reflection.md.tpl`（v0.9.0 加；個體層 entry 範本）
- `tools/post-upgrade-verify-spec.md` 模式 B/C 補完（v0.9.0 ship；模式 B 升版 diff / 模式 C pre-commit sync-check）
