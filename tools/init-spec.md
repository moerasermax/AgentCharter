...
7. domain_axioms.primary 指向的檔案實際存在（不只 schema 寫了路徑）+ frontmatter status 校驗（v0.8.0 加；signal #23 條款化）：
   - `status: USER-RATIFIED` → PASS
   - `status: AI-DRAFTED` → FAIL（不可在 AI-DRAFTED 啟動 init、依 v0.7.1 路徑 B 紀律 + `core/domain-axiom-slot §3.3`）
   - `status` 缺 / 其他值 → FAIL（違反 v0.7.1 雙路徑二態紀律）
...
| **v0.8.0** | **Phase 5b CHECK 7 擴含 axiom frontmatter status 校驗**（USER-RATIFIED → PASS / AI-DRAFTED → FAIL / 其他 → FAIL）。**觸發**：dogfood signal #23 條款化（v0.7.0 公司接入第一次失敗 + v0.7.6 LIVE 公司專案接入第二次同類觀察 — Gemini 路徑 B 寫 axiom AI-DRAFTED + user 跳過校正 + init Phase 1-5b 跑通 + Phase 5b CHECK 7 PASS = surface PASS / structural fail）。本次擴是 init 端 fail-fast 執行載體；對應 doctor §3.9 為任意時點驗證載體；post-upgrade-verify 軸 D D001 為升版專屬載體 — 三層雙重防禦對齊 v0.7.3 北極星「不讓 user 記」精神 | ✅ |
