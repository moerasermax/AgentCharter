...
5. 簽名 agent-commons/roles/<role>/_role.md（依 cross-ai-handoff §6 五欄切換歷史格式）
6. 回報使用者：建好了，請打 /<role>-init 驗證
```
...
請接「Engineer」角色，依以下流程自我具象化：

1. 讀 ~/.agentcharter/core/init-template.md §3.3.2 6 步驟
2. 讀 ~/.agentcharter/roles/engineer/_spec.md（職能定義）
3. 讀 ~/.agentcharter/roles/engineer/claude-code.md（你的 vendor spec）
4. 讀 ~/.agentcharter/core/{role-separation,audit-rights,evidence-first,
   structural-anti-fabrication,completion-delivery,working-stack-discipline}.md
5. 讀本專案 agent-commons/_config/{profile,mapping}.yaml
6. 讀本專案領域公理（mapping.yaml.domain_axioms.primary 指向的檔）

完成後：
- 在 .claude/commands/engineer-init.md 生成你的 slash command
- 簽名 agent-commons/roles/engineer/_role.md（依 cross-ai-handoff §6
  五欄格式）
- 通知我「我已建好 /engineer-init，請驗證」
```
...
完成後：
- 在 .gemini/commands/pm-init.toml 生成你的 slash command
- 簽名 agent-commons/roles/pm/_role.md
- 通知我「我已建好 /pm-init，請驗證」
```
