...
- **v1.22（2026-05-22，charter v0.11.0）** — 🔥 **Gemini CLI 棄用觸發 Antigravity CLI vendor 接入 MINOR 連動 sync**（dogfood signal #60 LIVE 條款化候選）。**升 v0.11.0 注意**：(a) Gemini CLI 免費 / Pro / Ultra tier 採用方 — **6/18 前必須遷移**（依 walkthrough 跑 10 步流程）；Enterprise / 付費 API key / OSS tier — 不必動；(b) 新 vendor spec `roles/pm/antigravity-cli.md` v1.1（path A AI-DRAFTED-FROM-GEMINI-V1.8 + 同日 LIVE 第二輪校正、Skills 路徑為 `~/.gemini/skills/<name>/SKILL.md` 子目錄結構 + frontmatter `name:` 必填）；(c) `roles/pm/gemini-cli.md` v1.9 標 LEGACY_AUTO_IMPORTED + §9 採用方遷移指引、既有內容不刪；(d) 工具能力差異對照：`gemini` binary → `agy` / `.gemini/commands/*.toml` → `~/.gemini/skills/<name>/SKILL.md` 子目錄 / Gemini family model → 任意 model（Claude / Gemini / GPT 多選、`/model` 切換）。**設計學意義**：A1 公理時運實證（PM 角色不死、Claude PM + Antigravity 雙保險）+ vendor 中立架構反向實證（v0.10.0 commit-hook 跨 vendor 不受影響、survive vendor death）。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md`](./examples/upgrades/v0.10.6-to-v0.11.0-antigravity-migration.md)**。詳見 CHANGELOG v0.11.0 段。

- **v1.21（2026-05-21，charter v0.10.6）** — Claude PM v1.0 接入 + Gemini PM v1.8 best-of-breed 收斂升級 PATCH 連動 sync。**零採用方動作要求**（純 vendor spec 新增 + 既有 vendor spec 擴增）— 升版只改 profile.yaml `charter_version: "0.10.5"` → `"0.10.6"`。**核心改動**：(a) 新檔 `roles/pm/claude-code.md` v1.0（path B dogfood 收編 pattern 第一次完整 LIVE 實證、含 §3 Claude 特化 2 row：Over-Conservative Self-Rule + dual-mode context cross-mode forgetting）；(b) `roles/pm/gemini-cli.md` v1.7 → v1.8（best-of-breed 收斂升級、§1 加 2 row + §3 加 dual-mode forgetting row + 新加 §7 vendor 接入回顧獨立段含 signal #5 + #55 LIVE 反例自報）；(c) `roles/pm/_spec.md §7` 對應 AI 表 update。**採用方接入影響**：PM 角色 vendor coverage 從 Gemini 單一擴為 Gemini + Claude 雙 vendor — `core/init-template §3.3` self-instantiation 時接 PM 的 AI 可選 Gemini CLI 或 Claude Code 任一、依採用方環境配置。**設計動機**：dogfood signal #59 LIVE 條款化候選 — `ai-vendor-onboarding §3 step 3` 原設計「first-mover baseline」假設失效、user explicit 授權走 (b) maintainer 代修反向 regression；對應 SSS S2.4 候選議程「跨 vendor 知識聚合 + 互為養分 + 收斂 best-of-breed」LIVE 第 1 次完整實證 + v0.7.3 北極星「培養魚塘、不討魚」精神。詳見 CHANGELOG v0.10.6 段。
- **v1.20（2026-05-10，charter v0.10.5）** — Gemini CLI 預設 `generalist` 自動分包處置 PATCH 連動 sync。**零採用方動作要求**（純 vendor spec 擴增）— 升版只改 profile.yaml `charter_version: "0.10.4"` → `"0.10.5"`。**核心改動**：`roles/pm/gemini-cli.md §3.5.5`（v1.6 → v1.7）新加段「Gemini CLI 預設 `generalist` 自動分包處置 — PM init 必提醒」 — PM self-instantiation 開始時必對採用方主動發 reminder（執行 `/agents disable generalist` slash command 對齊 `role-separation §3.5` 卡控紀律）+ 採用方回應後處置表 + `pm-init.toml` 自具象化要求 + §3.2 派發任務既有實作的設計衝突 follow-up 段。**採用方接入影響**：Gemini PM 接入時會主動建議執行 disable 命令、避免 generalist 自動分包繞 PM 卡控；採用方依紀律執行即可（不執行則 PM 後續每次任務派發前 self_audit 額外項）。**設計動機**：dogfood signal #55（user 直接條款化、CryptoBot 反向接入 LIVE 實證）— Gemini CLI vendor 預設行為層才是 PM 越矩真主因、之前 v0.6.0 signal #5 條款化以為治本「LLM 主動繞路」、實際 vendor 預設行為層才是真主因；對應 v0.8.2 §設計哲學第 5 條「弱保證項升結構強制」家族新類型（vendor 預設行為層紀律、雙軸座標第 4 軸）。詳見 CHANGELOG v0.10.5 段。
- **v1.19（2026-05-06，charter v0.10.4）** — Vendor 介紹 charter 工具紀律 charter common 化 PATCH 連動 sync。**零採用方動作要求** — 升版只改 profile.yaml `charter_version: "0.10.3"` → `"0.10.4"`。**核心改動**：(a) `core/ai-vendor-onboarding §3.5` 新加段「Vendor 介紹 charter 工具紀律」（三段流程：介紹用法 + 主動詢問「要現在幫你裝嗎？」+ user 同意直接跑、禁被動「如果您希望」表述）；(b) `tools/vendor/commons/README.md` 工具清單段 user-facing onboarding 化（白話 + 例子 + 一鍵 prompt）；(c) `roles/pm/gemini-cli.md §3.7` v1.5 → v1.6（觸發場景擴展 + 標 §3.5 charter common 紀律的 PM 實作落地）。**設計動機**：dogfood signal #53（user 直接條款化）+ #54 LIVE — Engineer Claude 介紹 /checkpoints 用「如果您希望」被動描述、user 不知怎麼裝、要去查文件。**對應 v0.7.3 北極星「不讓 user 記」延伸到「不讓 user 為了用一個工具還要查 charter 文件」**。詳見 CHANGELOG v0.10.4 段。
- **v1.18（2026-05-06，charter v0.10.3）** — 純 spec sweep + maintainer-only lint binary PATCH 連動 sync。**零採用方動作要求**（向下兼容、純文檔 / spec / maintainer 工具層擴增）— 升版只改 profile.yaml `charter_version: "0.10.2"` → `"0.10.3"`、無其他動作。**升版內容**：(a) `post-upgrade-verify-spec §3.1-§3.5` 全 5 軸 22 個 check item 升 spec-as-data 四欄結構（SSS S3 propagate 終局）；(b) `doctor-spec §3` 段首加全局「真實 stdout 證據要求」紀律段；(c) `README §設計哲學` 第 5 條加「升維軌跡」段（雙軸 framing 第三段、charter v0.5.7 起 11 個 hardening 循環 trajectory）；(d) 新加 `tools/charter-spec-lint.sh`（maintainer-only binary、L1 tool spec 四欄 + L2 core 雙軸 blockquote 偵測）；(e) `maintainer-discipline §3.1.1` 加 lint binary 紀律。**設計動機**：dogfood signal #46（≥ 3 次）+ #31（≥ 5 次同類）+ user LIVE 提問「升版自動納入」結構性解。**對應 v0.7.3 北極星「不讓 user 記」延伸到「不讓 maintainer 記」**。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.2-to-v0.10.3.md`](./examples/upgrades/v0.10.2-to-v0.10.3.md)**（採用方 1 步流程）。詳見 CHANGELOG v0.10.3 段。
- **v1.17（2026-05-06，charter v0.10.2）** — commit hook H7 schema-driven 強制必啟集合 **BREAKING-LITE PATCH** 連動 sync（含 v0.10.1 step 0.5 charter version 主動通知 sync 補齊）。**升 v0.10.2 注意**：⚠️ **BREAKING-LITE** — 升完若 profile.yaml 漏強制必啟欄位（當前 ship REQ-001-F6 = `enable_modes` 必含 F6）下次 commit 被 H7 binary 攔截、不可繞（除非走 `--no-verify`）。**升版 5 步**：(a) `git pull ~/.agentcharter` 拿新 `tools/profiles/_required.yaml` + hook v1.1；(b) `bash ~/.agentcharter/tools/vendor/commons/install-git-hooks.sh --update` 同步 deploy script；(c) 檢查 profile.yaml 對齊 `_required.yaml`（如 dbSDK case 漏 F6 → 補 F6）；(d) profile.yaml `charter_version: "0.10.1"` → `"0.10.2"`；(e) commit 測試 H7。**新概念**：(1) `tools/profiles/_required.yaml` 與 standard/strict/minimal/essential.yaml 並列、用途互補（preset 規範「啟用 / 不啟用」選擇空間、本檔規範「**不可不啟用**」核心強制必啟欄位）；(2) commit-hook H1-H7（v0.10.2 加 H7、schema-driven 設計、未來 F7 / F8 不需新加 hook，改 `_required.yaml` 加 entry + 補 5-10 行 inline check 即可）。**設計動機**：dogfood signal #46 + #31 + #52 候選累積（2026-05-06 公司專案 dbSDK LIVE 三層雙重防禦對 F6 強制必啟整體失效、Engineer/Gemini 兩個 vendor 跑 verify+doctor 同 session 連續 simulated PASS、user 親手抓 + 詰問「我們的 doctor 會驗證出來嗎」「以後 F7 一樣可以解嗎」）。**對應 v0.8.2 §設計哲學第 5 條「弱保證項升結構強制」最赤裸 LIVE 實證** + **dogfood signal #27「spec-driven 與 LLM 自律 循環依賴」結構性解**。**v0.10.1 step 0.5 配套補齊**（之前 v0.10.1 採用方文檔層漏 sync、本 entry 一併補完）：`core/init-template §3.3.2` 加 step 0.5（charter version 比對 + 主動通知）— 三分支處理（equal / framework>project INFO / framework<project ERROR）、對齊 v0.7.3 北極星「不讓 user 想到去比對版本」。**詳細 step-by-step 升版流程見 [`examples/upgrades/v0.10.1-to-v0.10.2.md`](./examples/upgrades/v0.10.1-to-v0.10.2.md)**（首個 BREAKING-LITE PATCH walkthrough）。詳見 CHANGELOG v0.10.2 + v0.10.1 段。
...
- **v1.11（2026-04-30，charter v0.9.0）** — 紀律完整性 + AI 自我覺察升維 MINOR 連動 sync（dogfood signal #34 LIVE 條款化）：line 6 charter 對應版本 v0.8.2 → v0.9.0。**升 v0.9.0 注意**：(a) 4 條新加 condition（`core/individual-learning-loop.md` 第 13 個架構級概念 + 補完接班場景四軸的第 4 軸 / `core/diagnose-remediate-protocol.md` SSS S3 架構級條款化 / `core/adoption-lifecycle.md` lifecycle 5 階段完整化 / `core/condition-mutability.md` 紀律本體）— standard/strict preset 自動含、採用方升版主要動作就是改 profile.yaml `charter_version: "0.8.2"` → `"0.9.0"`；(b) `core/init-template §3.3.2` 七步驟 → 八步驟（加 step 0「讀過去違反紀錄」對應個體學習迴圈 §3 讀紀律）— 既有 slash command 雖仍可用、但 step 0 漏跑 = 命中 F6 surface-level、強烈建議重新具象化；(c) `templates/agent-commons/reflection.md.tpl` 新範本（個體層反省範本、雙寫紀律執行載體 — 命中 F-mode 後 AI 必雙寫個體層 `roles/<role>/reflections/<date>_<f-mode>_*.md` + 集體層 `state/failure_mode_log.md`）；(d) `tools/doctor-spec.md §3.11` 新校驗（W1101 reflections/ 缺 / W1102 雙寫漏對應 / E1103 frontmatter 不全）；(e) 新加 essential preset（3-5 條 core / < 5k init token、`tools/profiles/essential.yaml`）— 探索期專案首選、signal #28 progressive adoption + signal #26 ROI 真槓桿；(f) 新 spec `tools/uninstall-spec.md` `/charter-uninstall` 棄用工具設計 —「保留最後的溫柔」精神（archive 報告 + 三 level Soft/Full/Nuclear）。**對應 dogfood-driven hardening 第十七循環**（charter 完成 v0.7.3 北極星閉環 — 對採用方 + AI 雙邊「不讓 user 記」）。**詳細 step-by-step 升版流程（含每步 AI prompt 範本）見 [`examples/upgrades/v0.8.2-to-v0.9.0.md`](./examples/upgrades/v0.8.2-to-v0.9.0.md)**。詳見 CHANGELOG v0.9.0 段。
- **v1.10（2026-04-30，charter v0.8.2）** — 雙軸矩陣 framing 第一段連動 sync（multi-perspective 第十四循環結構師金礦落地）：line 6 charter 對應版本 v0.8.1 → v0.8.2。**升 v0.8.2 注意**：純擴增 README + 21 條條款開頭 blockquote、既有採用方升版只改 profile.yaml `charter_version: "0.8.1"` → `"0.8.2"`。`README.md §設計哲學` 新加第 5 條「雙軸座標 — 哪些紀律靠誰守」（物理依據軸 + 檢測時點軸 + 依賴 LLM 紀律的條款清單）— 提供採用方看每條 charter 條款的保證強度的能力。21 條 `core/*.md` 開頭加 blockquote 三新行（保證強度 / 檢測時點 / since）— 對採用方執行邏輯零影響、純文檔層擴增。**詳細 step-by-step 升版流程（含每步給 AI 的 prompt 範本）見 [`examples/upgrades/v0.8.1-to-v0.8.2.md`](./examples/upgrades/v0.8.1-to-v0.8.2.md)** — charter 升版系列最簡 walkthrough（2 步流程）+ §5 跨版本場景指引。詳見 CHANGELOG v0.8.2 段。
